  ...
  for (let r = 0; r < BITS; r++) {
    for (let g = 0; g < BITS; g++) {
      for (let b = 0; b < BITS; b++) {
        const rr = (r * 255) / (BITS - 1)
        const gg = (g * 255) / (BITS - 1)
        const bb = (b * 255) / (BITS - 1)

        const mid = (rr * 30 + gg * 59 + bb * 11) / 100

        const r1 = ~~((((rr + mid * 1) / 2) * 230) / 255 + 10)
        const g1 = ~~((((gg + mid * 1) / 2) * 230) / 255 + 10)
        const b1 = ~~((((bb + mid * 1) / 2) * 230) / 255 + 10)
